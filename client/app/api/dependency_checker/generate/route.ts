...
export async function POST(req: NextRequest) {
    try{
        ...
        if (data.lang === 'java') {
            ...
            cmd = ["sh", "-c", "git clone https://github.com/$USER/$REPO.git && python java_library_usage.py && tail -f /dev/null"];
        } else {
            binds = [parentDir + `/dep_checker_py:/app`]
            cmd = ["sh", "-c", "git clone https://github.com/$USER/$REPO.git && python python_library_usage.py && tail -f /dev/null"];
        }
